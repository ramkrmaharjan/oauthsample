...
/**
 * Task resource that returns Google tasks that was queried using a
 * {@link Client}.
 *
...
public class TaskResource {
	...
	public Response getTasks() {
		...
		final Client client = SimpleOAuthService.getFlow()
				.getAuthorizedClient();
		System.out.println("Get Task Response");
	...
	private Response authRedirect() {
		...
		OAuth2CodeGrantFlow flow = builder
				.property(OAuth2CodeGrantFlow.Phase.AUTHORIZATION, "readOnly",
						"true").redirectUri(redirectURI).scope("contact")
				.build();

		SimpleOAuthService.setFlow(flow);

		// start the flow
		final String authURI = flow.start();

		// redirect user to Google Authorization URI.
		return Response.seeOther(UriBuilder.fromUri(authURI).build()).build();
	}
	 ...
	 *
	 * @param client
	 *            Client configured for authentication with access token.
	 * @return Google task data response or redirect to google authorize page
	 *         response.
	 */
	// FIXME: This code will not be called now :Changed Ram: 5:35PM -> 3/21/2015
	private Response getTasksResponse(final Client client) {
		client.register(JacksonFeature.class);
		client.register(new LoggingFilter(Logger
				.getLogger("example.client.tasks"), true));

		final WebTarget baseTarget = client.target(GOOGLE_TASKS_BASE_URI);
		final Response response = baseTarget.path("users/@me/lists").request()
				.get();
		...
		case 200: // Response.Status.OK
			listOfTaskLists = processTaskLists(baseTarget,
					response.readEntity(TaskRootBean.class));
			break;
		default:
	 ...
	 * @param taskRootBean
	 *            root task bean to be processed
	 * @return Detailed list of non-completed tasks or {@code null} if there is
	 *         no task list available.
	 */
	private List<TaskListModel> processTaskLists(final WebTarget baseTarget,
			final TaskRootBean taskRootBean) {
		...
		for (final TaskListBean taskListBean : taskRootBean.getItems()) {
			...
			final WebTarget listTarget = baseTarget.path(
					"lists/{tasklist}/tasks").resolveTemplate("tasklist",
					taskListBean.getId());

			final TaskListBean fullTaskListBean = listTarget.request().get(
					TaskListBean.class);
			...
			listOfTaskLists.add(new TaskListModel(taskListBean.getTitle(),
					taskList.size() > 0 ? taskList : null));
		}
